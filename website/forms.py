...
class RegisterForm(FlaskForm):
    ...
    address = TextAreaField("House Address", validators=[Length(1, 255), InputRequired()])
...
class CreateEventForm(FlaskForm):
    ...
    status = SelectField(
        "Status",
        choices=[("Open"), ("Inactive"), ("Sold Out"), ("Cancelled")],
        validators=[InputRequired()],
    )
    type = SelectField(
        "Genres",
        choices=[("Live"), ("Recorded"), ("Workshop"), ("Lecture")],
        validators=[InputRequired()],
    )
    image = FileField(
        "Event Image",
        validators=[
            FileRequired(message="Image Required"),
